					...
					if (field.getType().getInterfaces().length > 0) {
						for (Class<?> interfaces: field.getType().getInterfaces()) {
							if (interfaces == AbstractConstant.class) {
								BeanItemContainer<AbstractConstant> bc = new BeanItemContainer<AbstractConstant>(AbstractConstant.class);
								Method valuesMethod = field.getType().getDeclaredMethod("values", null);
								AbstractConstant[] values = (AbstractConstant[]) valuesMethod.invoke(null, null);
